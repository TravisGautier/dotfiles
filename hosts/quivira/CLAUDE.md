...
Host-specific guidance for quivira, the primary Arch Linux workstation. See the repo-root `CLAUDE.md` for the multi-host layout and shared conventions.
...
This host uses **symlinks** for config management:
- `~/.config/hypr` → `~/dotfiles/hosts/quivira/.config/hypr` (symlink)
- `~/.gitconfig` → `~/dotfiles/hosts/quivira/.gitconfig` (symlink)
- Other shell dotfiles and `.config` dirs are copies, synced by the `/hyprgit` skill
- `etc/` configs are tracked but require manual `sudo cp` to apply

Run `./install.sh` (in this directory) to set up symlinks on a new system.
...
See `CHANGELOG.md` in this directory for configuration history and troubleshooting sessions. **Always update changelog before committing significant changes.**
